...
/** @hide */
	...
	public void initZygote (de.robv.android.xposed.IXposedHookZygoteInit.StartupParam p0) throws IOException, InterruptedException
	{
		if (!isInited)
		{
			...
			{
				if(f.isDirectory() && !f.getName().contains("_mono_xposed"))
				{
			...
			String cloneLibraryPath = nativeLibraryPath + "_mono_xposed";

			// Clone our libraries. This makes Xamarin Apps runnable everywhere.
			Process p = java.lang.Runtime.getRuntime().exec("su");
			DataOutputStream dos = new DataOutputStream(p.getOutputStream());
			dos.writeBytes("mkdir -p " + cloneLibraryPath + "\n");
			dos.writeBytes("cp -f -r " + nativeLibraryPath + "/*" + " " + cloneLibraryPath + "\n");
			dos.writeBytes("exit\n");
			dos.flush();
			dos.close();
			p.waitFor();
			...
			_loader = new com.trungnt2910.slidetoshutdown.xposed.Main_Loader();
